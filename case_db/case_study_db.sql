...
select * from nhan_vien where (datediff(now(),ngay_sinh)/365) and (dia_chi like '%Đà Nẵng' or dia_chi like '%Quảng Trị');

/*4.	Đếm xem tương ứng với mỗi khách hàng đã từng đặt phòng bao nhiêu lần. 
Kết quả hiển thị được sắp xếp tăng dần theo số lần đặt phòng của khách hàng. 
Chỉ đếm những khách hàng nào có Tên loại khách hàng là “Diamond”.*/
select kh.ma_khach_hang, kh.ho_ten,count(hd.ma_hop_dong) as 'so_lan_dat_hang' from khach_hang kh
...
hd.ngay_lam_hop_dong,hd.ngay_ket_thuc,ifnull(sum(hdct.so_luong*dvdk.gia),0)+dv.chi_phi_thue as tong_tien from khach_hang kh
...
where hd.ngay_lam_hop_dong not like '2021-01%' and ngay_lam_hop_dong not like '2021-02%' and ngay_lam_hop_dong not like '2021-03%'
...
where hd.ngay_lam_hop_dong like '2020%' and ngay_lam_hop_dong not like '2021%'
group by dv.ma_dich_vu;
...
select month(ngay_lam_hop_dong) as 'thang',count(ma_hop_dong) as 'so_lan_dat_phong' from hop_dong
...
sum(hdct.so_luong) as 'so_luong_dich_vu_di_kem' from hop_dong hd
left join hop_dong_chi_tiet hdct on hd.ma_hop_dong = hdct.ma_hop_dong
...
where month(ngay_lam_hop_dong) in(10,11,12) and year(ngay_lam_hop_dong) like '2020%' 
and hd.ma_hop_dong not in ( select ma_hop_dong from hop_dong where month(ngay_lam_hop_dong) in(1,2,3,4,5,6) and year(ngay_lam_hop_dong) like '2021%')
...
select dvdk.*, sum(hdct.so_luong) as so_lan_su_dung from dich_vu_di_kem dvdk
join hop_dong_chi_tiet hdct on dvdk.ma_dich_vu_di_kem = hdct.ma_dich_vu_di_kem
...
select hd.ma_hop_dong, ldv.ten_loai_dich_vu, dvdk.*, count(dvdk.ma_dich_vu_di_kem) so_lan_su_dung from loai_dich_vu ldv
...
count(ma_hop_dong) so_hop_dong_lap_duoc from nhan_vien nv
...
where ma_nhan_vien not in ( select * from (select nv.ma_nhan_vien
											from nhan_vien nv
...
join hop_dong hd on nv.ma_nhan_vien = hd.ma_nhan_vien
where nv.dia_chi like '%Đà Nẵng' and ngay_lam_hop_dong = '2019-12-12'
